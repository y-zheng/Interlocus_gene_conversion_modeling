...
gen.analysis.2copy = function (pop) {
 ...
 freqs = matrix(0,length(pop$pos),2) #Frequency by count, each copy separately
 ...
 ovec = matrix(0,8,16) #Three vectors for three types of sites
 
 for (j in 1:8) {
  ovec[j,1:3] = c(rr,gen,sum(hlc2))
  ...
  if (sum(loc.ty==j) == 1) {
   ...
   ovec[j,8] = 2*freqs.total[vtemp]*(freqs.total[1]-freqs.total[vtemp])/(freqs.total[1]^2)
   ovec[j,9] = 1/sum(1/(1:freqs.total[1]))
   ovec[j,10] = 2*freqs[vtemp,1]*(4000-freqs[vtemp,1])/(4000^2)
   ovec[j,11] = varsites[vtemp,1]/8.87139
   ovec[j,12] = 2*freqs[vtemp,2]*(freqs[1,2]-freqs[vtemp,2])/(freqs[1,2]^2)
   ovec[j,13] = varsites[vtemp,2]/sum(1/(1:freqs[1,2]))
   ovec[j,14] = mean(pop$sites[hlc2,vtemp,1] != pop$sites[hlc2,vtemp,2])
   hs = ((2*freqs[vtemp,1]*(4000-freqs[vtemp,1])/4000) + (2*freqs[vtemp,2]*(freqs[1,2]-freqs[vtemp,2])/freqs[1,2]))/(4000+freqs[1,2])
   ht = 2*(freqs[vtemp,1]+freqs[vtemp,2])*(4000+freqs[1,2]-freqs[vtemp,1]-freqs[vtemp,2])/((4000+freqs[1,2])^2)
   if (ht > 0) {
    ovec[j,15] = (ht-hs)/ht
    ovec[j,16] = (ht-hs)/ht
   }
  ...
  if (sum(loc.ty==j) > 1) {
   ovec[j,4] = sum(loc.ty==j)
   ovec[j,5:6] = apply(varsites[(loc.ty==j),],2,sum)
   ovec[j,7] = sum(varsites[(loc.ty==j),1]*varsites[(loc.ty==j),2])
  
   ovec[j,8] = sum(2*freqs.total[(loc.ty==j)]*(freqs.total[1]-freqs.total[(loc.ty==j)])/(freqs.total[1]^2))
   ovec[j,9] = sum(loc.ty==j)/sum(1/(1:freqs.total[1]))
   ovec[j,10] = sum(2*freqs[(loc.ty==j),1]*(4000-freqs[(loc.ty==j),1])/(4000^2))
   ...
   hs = ((2*freqs[(loc.ty==j),1]*(4000-freqs[(loc.ty==j),1])/4000) + (2*freqs[(loc.ty==j),2]*(freqs[1,2]-freqs[(loc.ty==j),2])/freqs[1,2]))/(4000+freqs[1,2])
   ht = 2*(freqs[(loc.ty==j),1]+freqs[(loc.ty==j),2])*(4000+freqs[1,2]-freqs[(loc.ty==j),1]-freqs[(loc.ty==j),2])/((4000+freqs[1,2])^2)
   fsttemp = (ht >0)
   hs = hs[fsttemp]
   ht = ht[fsttemp]
...
outhead = paste("Round3.5_NL/Round3.5NL_purifying_",pur.sel,"_GC_",gcr,"_length_",gcml,"_pnlty_",gc.log,"_rep_",rr,sep="")
...
nuc.st = rep(3,20000) #Intron sizes: 3000, 1200, 600, 300, 60
...
in.pos = read.table(paste("Round3.5_Initial/Round3.5_newlocus_inieq_pursel_",pur.sel,"_rep_",rr,"_g40k_pos.txt",sep=""),header=FALSE)[,1]

newsites.3d = array(0,dim=c(2*ne,length(in.pos),2))
newsites.3d[,,1] = in.sites
 
pop.x = list(sites=newsites.3d,pos=in.pos,fit=rep(1,ne),cpn=rep(1,2*ne),rec=c(0,0.005))
#Read pop end
